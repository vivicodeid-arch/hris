@extends('layouts.app')
@section('titlepage', 'Set User Permission')
    ...
    <div class="d-flex justify-content-between align-items-center w-100">
        <div>
            ...
            <div class="text-muted mt-1" style="font-size: 0.75rem; font-weight: normal; text-transform: none; letter-spacing: 0px;">
                Mengatur hak akses langsung (direct permissions) untuk user <strong>{{ $user->name }}</strong>.
            </div>
        </div>
            ...
            <ol class="breadcrumb breadcrumb-style1 mb-0">
                ...
                <li class="breadcrumb-item">
                    <a href="{{ route('users.index') }}">
                        <i class="ti ti-user ti-xs me-1"></i> Users
                    </a>
                </li>
...
<div class="row align-items-center mb-4">
    <div class="col-12 d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-3">
        <h4 class="mb-0 fw-bold">
            Direct Permissions for <span class="text-primary">{{ $user->name }}</span>
        </h4>
                    ...
                    <div class="card-body pt-4">
                        ...
                        @foreach ($list_permissions as $p)
                             @php
                                 ...
                                 $cek = in_array($permission_name, $userpermissions);
                                 $rolesGranting = $permissionRoleMap[$permission_name] ?? null;
                             @endphp
                             <div class="form-check form-switch mb-3">
                                 <input class="form-check-input permission-checkbox" type="checkbox" name="permission[]"
                                     ...
                                     {{ $cek > 0 ? 'checked' : '' }} {{ !empty($rolesGranting) ? 'disabled' : '' }} style="cursor: pointer;">
                                 <label class="form-check-label text-secondary mt-1 ms-1" for="defaultCheck{{ $permission_id }}" style="font-size: 0.9rem; cursor: pointer;">
                                     {{ $permission_name }}
                                     @if(!empty($rolesGranting))
                                         <span class="badge bg-label-info ms-1" style="font-size: 0.65rem; padding: 2px 4px; font-weight: normal; vertical-align: middle;" title="Inherited from role(s): {{ implode(', ', $rolesGranting) }}">
                                             Role: {{ implode(', ', $rolesGranting) }}
                                         </span>
                                     @endif
                                 </label>
                             </div>
            ...
            <div class="card shadow-none border mb-4 bg-white">
                <div class="card-body d-flex flex-column flex-sm-row justify-content-between align-items-center py-3 gap-3">
                    <div class="text-center text-sm-start">
                        <span class="text-muted"><i class="ti ti-info-circle me-1 text-primary"></i> Hak akses yang dicentang di sini akan menjadi hak akses spesifik (langsung) yang dimiliki oleh user bersangkutan secara pribadi.</span>
                    </div>
                    <div class="d-flex gap-2">
                        <a href="{{ route('users.index') }}" class="btn btn-label-secondary px-4 fw-semibold">Kembali</a>
