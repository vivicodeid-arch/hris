<form action="{{ route('kontrak.update', Crypt::encrypt($kontrak->id)) }}" method="POST" id="formKontrakEdit" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="row">
        ...
        <div class="form-group mb-3">
            <label for="dokumen" style="font-weight: 600" class="form-label">Upload Dokumen Kontrak Baru (PDF, Opsional)</label>
            <input class="form-control" type="file" id="dokumen" name="dokumen" accept=".pdf">
            @if ($kontrak->dokumen)
                <div class="mt-1">
                    <a href="{{ asset('storage/uploads/kontrak/' . $kontrak->dokumen) }}" target="_blank" class="text-primary fs-7">
                        <i class="ti ti-file-text me-1"></i> Lihat Dokumen Saat Ini ({{ $kontrak->dokumen }})
                    </a>
                </div>
            @endif
            @error('dokumen')
...
<script>
    $(function() {
        ...
        $("#formKontrakEdit").on('submit', function(e) {
            ...
            const fileInput = $('#dokumen')[0];
            if (fileInput && fileInput.files.length > 0) {
                const file = fileInput.files[0];
                const fileExtension = file.name.split('.').pop().toLowerCase();
                if (fileExtension !== 'pdf') {
                    errors.push('Format dokumen kontrak harus PDF');
                }
            }
