...
<head>
    <meta charset="utf-8">
    <title>Formulir Pengajuan Reimbursement - {{ $reimbursement->nama_karyawan }} - {{ $reimbursement->no_reimbursement }}</title>
...
<body>
    ...
    <div class="wrapper">
        ...
        <!-- Title -->
        <div class="doc-title">Formulir Pengajuan Reimbursement</div>
        <div class="doc-number">Nomor Dokumen: {{ $reimbursement->no_reimbursement }}</div>
        ...
        <table class="details-table">
            <tr>
                ...
                <td class="value">{{ $reimbursement->nik_show ?? $reimbursement->nik }}</td>
            </tr>
            <tr>
                <td class="label">Nama Lengkap Karyawan</td>
                <td class="separator">:</td>
                <td class="value">{{ $reimbursement->nama_karyawan }}</td>
            </tr>
            <tr>
                <td class="label">Jabatan / Posisi</td>
                <td class="separator">:</td>
                <td class="value">{{ $reimbursement->nama_jabatan ?? '-' }}</td>
            </tr>
            <tr>
                <td class="label">Departemen / Divisi</td>
                <td class="separator">:</td>
                <td class="value">{{ $reimbursement->nama_dept }}</td>
            </tr>
            <tr>
                <td class="label">Cabang Penempatan</td>
                <td class="separator">:</td>
                <td class="value">{{ $reimbursement->nama_cabang }}</td>
            </tr>
        </table>
        ...
        <table class="details-table">
            <tr>
                ...
                <td class="separator">:</td>
                <td class="value"><strong>{{ date('d-m-Y', strtotime($reimbursement->tanggal_pengajuan)) }}</strong></td>
            </tr>
            <tr>
                ...
                <td class="value">
                    @if ($reimbursement->status == 'P')
                        Pending
                    @elseif ($reimbursement->status == 'A')
                        Approved
                    @elseif ($reimbursement->status == 'R')
                        Rejected
                    @elseif ($reimbursement->status == 'D')
                        Dibayar
                    @elseif ($reimbursement->status == 'B')
                        Batal
                    @endif
                </td>
            </tr>
            ...
            <tr>
                <td class="label">Keterangan Umum</td>
                <td class="separator">:</td>
                <td class="value">{{ $reimbursement->catatan }}</td>
            </tr>
            @endif
        </table>

        <!-- Section 3: Rincian Klaim -->
        <div class="section-title">Rincian Item Biaya</div>
        <table class="items-table">
            <thead>
                <tr>
                    <th style="width: 5%;" class="text-center">No</th>
                    <th style="width: 15%;" class="text-center">Tanggal Item</th>
                    <th style="width: 25%;">Kategori Reimbursement</th>
            ...
            <tbody>
                @foreach ($details as $d)
                    <tr>
                        ...
                        <td class="text-center">{{ date('d-m-Y', strtotime($d->tanggal_transaksi)) }}</td>
                        <td>{{ $d->nama_jenis }}</td>
                        <td>{{ $d->keterangan }}</td>
            ...
            <tfoot>
                <tr style="font-weight: bold; background-color: #f8fafc;">
                    ...
                    <td class="text-right">Rp {{ number_format($reimbursement->total_nominal, 0, ',', '.') }}</td>
                </tr>
            </tfoot>
        </table>
        ...
        @if($approvals->count() > 0)
            <div class="section-title">Jejak Persetujuan / Approval</div>
            <table class="items-table">
                <thead>
                    <tr>
                        <th style="width: 10%;" class="text-center">Tahap</th>
                        <th style="width: 25%;">Pemroses</th>
                        <th style="width: 15%;" class="text-center">Keputusan</th>
                        <th style="width: 20%;" class="text-center">Waktu Proses</th>
                ...
                <tbody>
                    @foreach($approvals as $app)
                        <tr>
                            <td class="text-center">Tahap {{ $app->level }}</td>
                            <td>{{ $app->user_name }}</td>
                            ...
                            <td class="text-center">{{ date('d-m-Y H:i', strtotime($app->created_at)) }}</td>
        ...
        <table class="signature-table">
            <tr>
                <td>
                    ...
                    <div class="signature-name">{{ $reimbursement->nama_karyawan }}</div>
