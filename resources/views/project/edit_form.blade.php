<form action="{{ route('project.update', Crypt::encrypt($project->id)) }}" method="POST" id="formEditProject">
    @csrf
    @method('PUT')
    ...
    <div class="row">
        ...
        <div class="col-lg-6 col-md-12">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" name="nama_project" id="nama_project" value="{{ old('nama_project', $project->nama_project) }}" placeholder="Contoh: Pembuatan Website E-Commerce" required />
            </div>

            <div class="mb-3">
                <label class="form-label fw-bold" for="deskripsi">Deskripsi Project</label>
                <textarea class="form-control" name="deskripsi" id="deskripsi" rows="4" placeholder="Masukkan deskripsi detail mengenai project...">{{ old('deskripsi', $project->deskripsi) }}</textarea>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-6 mb-3">
                    ...
                    <select class="form-select" name="prioritas" id="prioritas" required>
                        <option value="low" {{ old('prioritas', $project->prioritas) === 'low' ? 'selected' : '' }}>RENDAH (LOW)</option>
                        <option value="medium" {{ old('prioritas', $project->prioritas) === 'medium' ? 'selected' : '' }}>SEDANG (MEDIUM)</option>
                        <option value="high" {{ old('prioritas', $project->prioritas) === 'high' ? 'selected' : '' }}>TINGGI (HIGH)</option>
                        <option value="critical" {{ old('prioritas', $project->prioritas) === 'critical' ? 'selected' : '' }}>KRITIS (CRITICAL)</option>
                    </select>
                </div>
            </div>
                ...
                <div class="col-6 mb-3">
                    ...
                    <select class="form-select" name="kode_dept" id="kode_dept">
                        ...
                        @foreach ($departements as $dept)
                            <option value="{{ $dept->kode_dept }}" {{ old('kode_dept', $project->kode_dept) == $dept->kode_dept ? 'selected' : '' }}>{{ strtoupper($dept->nama_dept) }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-6 mb-3">
                    ...
                    <select class="form-select" name="kode_cabang" id="kode_cabang">
                        ...
                        @foreach ($cabangs as $cab)
                            <option value="{{ $cab->kode_cabang }}" {{ old('kode_cabang', $project->kode_cabang) == $cab->kode_cabang ? 'selected' : '' }}>{{ strtoupper($cab->nama_cabang) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-6 mb-3">
                    ...
                    <div class="input-group input-group-merge">
                        ...
                        <input type="text" class="form-control flatpickr-date" name="start_date" id="start_date" value="{{ old('start_date', $project->start_date->format('Y-m-d')) }}" required placeholder="Tanggal Mulai" />
                    </div>
                </div>
                <div class="col-6 mb-3">
                    ...
                    <div class="input-group input-group-merge">
                        <span class="input-group-text"><i class="ti ti-calendar"></i></span>
                        <input type="text" class="form-control flatpickr-date" name="end_date" id="end_date" value="{{ old('end_date', $project->end_date->format('Y-m-d')) }}" required placeholder="Tanggal Selesai (Deadline)" />
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-6 col-md-12">
            ...
            <div class="row">
                ...
                <div class="col-6 mb-3">
                    ...
                    <div class="input-group input-group-merge">
                        ...
                        <input type="text" class="form-control" name="budget" id="budget" value="{{ old('budget', $project->budget ? formatRupiah($project->budget) : '') }}" placeholder="Contoh: 50.000.000" style="text-align: right;" />
                    </div>
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select select-search" name="leader_nik" id="leader_nik" required>
                    ...
                    @foreach ($karyawan as $k)
                        <option value="{{ $k->nik }}" {{ old('leader_nik', $leaderNik) == $k->nik ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <select class="form-select select-search-multiple" name="members[]" id="members" multiple style="min-height: 120px;">
                    @foreach ($karyawan as $k)
                        <option value="{{ $k->nik }}" {{ in_array($k->nik, old('members', $memberNiks)) ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" name="catatan" id="catatan" rows="3" placeholder="Masukkan catatan tambahan jika diperlukan...">{{ old('catatan', $project->catatan) }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="row mt-3 border-top pt-3">
        <div class="col-12 d-flex justify-content-end">
            ...
            @else
                <a href="{{ route('project.show', Crypt::encrypt($project->id)) }}" class="btn btn-label-secondary me-2">Batal</a>
            @endif
            <button type="submit" class="btn btn-primary"><i class="ti ti-device-floppy me-1"></i> Update Project</button>
        </div>
    </div>
</form>
...
<script>
    $(function() {
        ...
        function formatInput(el) {
            ...
            if(val) {
                el.val(new Intl.NumberFormat('id-ID').format(val));
            } else {
                el.val('');
            }
        }
        ...
        if ($('#leader_nik').length) {
            $('#leader_nik').select2({
                dropdownParent: $('#modalEditProject').length ? $('#modalEditProject') : null,
                width: '100%'
            });
        }
