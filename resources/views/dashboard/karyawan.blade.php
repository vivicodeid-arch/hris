...
@if(session()->has('impersonator_id'))
    <div class="bg-amber-100 border-b border-amber-200 px-4 py-3 text-amber-900 flex justify-between items-center text-sm font-medium sticky top-0 z-50">
        <div class="flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-amber-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
            <span>Login as: <span class="font-bold">{{ auth()->user()->name }}</span></span>
        </div>
        <a href="{{ route('users.stopimpersonate') }}" class="bg-amber-500 hover:bg-amber-600 text-white text-xs px-3 py-1.5 rounded-lg font-bold transition duration-150 shadow-none">
            Back
        </a>
    </div>
@endif
