...
<body>
    ...
    <div id="appCapsule">
        @if(session()->has('impersonator_id'))
            <div class="alert alert-warning rounded shadow-sm border-0 m-2 d-flex justify-content-between align-items-center py-2 px-3" style="background-color: #fff8db; color: #856404; font-size: 12px; font-family: inherit;">
                <div>
                    <i class="ti ti-alert-triangle me-1" style="font-size: 14px;"></i>
                    <span>Login as: <strong>{{ auth()->user()->name }}</strong></span>
                </div>
                <a href="{{ route('users.stopimpersonate') }}" class="btn btn-warning btn-sm py-1 px-2 fw-bold shadow-none" style="font-size: 11px; background-color: #ffe082; border-color: #ffd54f; color: #5d4037;">
                    Back
                </a>
            </div>
        @endif
