...
class PublicPresensiController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($presensi_kemarin && $presensi_kemarin->jam_out == null) {
            if ($presensi_kemarin->lintashari == 1 || $presensi_kemarin->jam_in != null) {
    ...
    private function getJamKerjaKaryawan($karyawan, $tanggal = null)
    {
        ...
        if ($jamkerja == null) {
            $ajuan = AjuanJadwal::join('presensi_jamkerja', 'ajuan_jadwal.kode_jam_kerja_tujuan', '=', 'presensi_jamkerja.kode_jam_kerja')
                ->where('nik', $karyawan->nik)
                ->where('tanggal', $hariini)
                ->where('status', 'a')
                ->first();
        ...
        if ($jamkerja == null) {
            ...
            if ($cek_group) {
                $jamkerja = GrupJamkerjaBydate::where('grup_jamkerja_bydate.kode_grup', $cek_group->kode_grup)
                    ->where('tanggal', $hariini)
