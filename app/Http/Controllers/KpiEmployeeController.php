...
class KpiEmployeeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('kpi.transactions.index', compact('karyawan', 'departemen', 'cabang', 'jabatan', 'active_period'));
    }
    ...
    public function show(string $id)
    {
         ...
         $user = auth()->user();
         if (!$user->isSuperAdmin()) {
             if (!in_array($kpi_employee->karyawan->kode_cabang, $user->getCabangCodes()) || !in_array($kpi_employee->karyawan->kode_dept, $user->getDepartemenCodes())) {
                 abort(403, 'Akses ditolak.');
             }
         }
    ...
    {
         $kpi_employee = KpiEmployee::with('karyawan')->findOrFail($id);
         $user = auth()->user();
         if (!$user->isSuperAdmin()) {
             if (!in_array($kpi_employee->karyawan->kode_cabang, $user->getCabangCodes()) || !in_array($kpi_employee->karyawan->kode_dept, $user->getDepartemenCodes())) {
                 abort(403, 'Akses ditolak.');
